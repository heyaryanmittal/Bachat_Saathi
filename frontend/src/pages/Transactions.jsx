    ...
    <div className="space-y-6 animate-entrance pb-12 overflow-x-hidden pt-2">
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-2">
            <Button 
                variant="secondary" 
                className={`w-full sm:w-auto flex items-center justify-center gap-2 py-3.5`}
                onClick={() => setShowFilters(!showFilters)}
            >
                <Filter className="w-4 h-4 text-primary" />
                <span className="font-black text-xs uppercase tracking-widest">{showFilters ? 'Hide Filters' : 'Filter Transactions'}</span>
            </Button>
            <Button onClick={() => setIsCreating(true)} className="btn-saas-primary shadow-lg shadow-primary/20 w-full sm:w-auto" size="lg"><Plus className="mr-2 w-5 h-5" />New Transaction</Button>
        </div>

        {/* Collapsible Filter UI (covers full width above the table) */}
        {showFilters && (
            <Card variant="glass" className="w-full animate-entrance">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 items-end">
                    ...
                    />
                    <Button variant="secondary" className="w-full font-black text-[10px] uppercase tracking-widest h-[42px] flex items-center justify-center" onClick={() => setFilters({ walletId: '', type: '', category: '', startDate: '', endDate: '' })}>Reset Filters</Button>
                </div>
            </Card>
        )}

        {/* Full-width Transactions Table Panel */}
        <Card className="p-0 overflow-hidden saas-card w-full">
